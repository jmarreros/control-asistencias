...
### Cron en producción (una sola línea, cubre todas las tareas programadas)
```
...
- **Vista `/logs`**: lista paginada (50 por página), filtros Todos/Admin/Portal, botón "Borrar" que elimina todos los registros
- **Purga automática**: `logs:purge --days=60` programado diariamente vía scheduler — los logs se eliminan automáticamente a los 60 días
...
app/
  Http/
    Controllers/
      AccessLogController.php        ← index() paginado con filtro tipo; destroy() borra todos los logs
      ...
      PinController.php              ← auth admin por PIN; registra login/logout/login_failed en AccessLog
      ...
      StudentPortalController.php    ← publicSearch() + lookup() (portal público sin auth); registra dni_lookup en AccessLog
    Middleware/
      CheckPin.php                   ← alias check.pin
      CheckSessionTimeout.php        ← alias session.timeout; expira sesión admin tras 8h de inactividad
      CheckStudentAuth.php           ← alias check.student (en desuso)
      LogAdminAccess.php             ← alias log.access; registra page_visit para páginas principales GET
  Models/
    Student.php · Clase.php · Attendance.php · StudentPlan.php · Setting.php · AccessLog.php
  Console/Commands/
    PurgeAccessLogs.php              ← logs:purge {--days=60}; programado diariamente en routes/console.php
  Exports/
...
resources/
  ...
  views/
    ...
    settings/edit.blade.php          ← precios + promociones + reportes (show_earnings) + notificaciones WA + PIN + logs + importar
    logs/
      index.blade.php                ← lista paginada de access_logs; filtros tipo; modal borrar con aviso purga 60 días
    reports/
...
routes/web.php
routes/console.php                   ← Schedule::command('logs:purge --days=60')->daily()
bootstrap/app.php                    ← alias middleware check.pin, session.timeout, check.student, log.access
